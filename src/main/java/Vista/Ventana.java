...
import ModeloDatos.EscribirBDHibernate;
import ModeloDatos.HibernateUtil;
import ModeloDatos.LeerBDHibernate;
...
public class Ventana extends javax.swing.JFrame {
    ...
    private EscribirBDHibernate crearFichero = new EscribirBDHibernate();
    private LeerBDHibernate leerFichero = new LeerBDHibernate();
    /**
    ...
    public void CargarArraysDeDatos(){
        ...
        listaEmpresas = leerFichero.LeerFicheroEmpresa();
        listaSagas = leerFichero.LeerFicheroSaga();
        listaVideojuegos = leerFichero.LeerFicheroVideojuego();
        listaPersonajes = leerFichero.LeerFicheroPersonaje();
        //System.out.println("tam: " + listaEmpresas.size());
    }
    ...
    private void BorrarPersonajeDelArray(){
        ...
        Personaje personajeBorrado = this.listaPersonajes.get(seleccionar);
        this.listaPersonajes.remove(seleccionar);
        MostrarTablaPersonaje();
        crearFichero.EliminarPersonaje(personajeBorrado);
    }
    private void jTable_personajeMouseClicked(java.awt.event.MouseEvent evt){
        ...
        for(i = 0; i < this.listaVideojuegos.size(); i++){
            if(this.listaVideojuegos.get(i).getIdVideojuego() == this.listaPersonajes.get(seleccionar).getIdVideojuegoPertenece().getIdVideojuego()){
    ...
    private void ExtraerDatosPersonaje(){
        ...
        Videojuego idVideojuego = null;
        for(int i = 0; i < listaVideojuegos.size(); i++){
            
            if(listaVideojuegos.get(i).getNombreVideojuego().equals(jComboBox_videojuego.getSelectedItem())){
                
                idVideojuego = listaVideojuegos.get(i);
            }    
        }
        
        Personaje personajeAñadido = new Personaje(nombrePersonaje, interpreteModelo, interpreteVoz, primeraAparicion, idVideojuego);
    ...
    private void jButton_guardar_personajeActionPerformed(java.awt.event.ActionEvent evt){
        ...
        CambiarEdiccionPersonaje(false);
        MostrarTablaPersonaje();
        this.crearFichero.AniadirOModificarPersonaje(listaPersonajes);
        seleccionar = -1;
        this.CargarDatosPersonajeEnVista();
    }
    ...
    private void BorrarVideojuegoDelArray(){
        ...
        for(int i = 0; i < listaPersonajes.size(); i++){
            if(listaPersonajes.get(i).getIdVideojuegoPertenece().getIdVideojuego() == listaVideojuegos.get(seleccionar).getIdVideojuego()){
        ...
        if(posibilidadBorrado){
            videojuegoEliminado = this.listaVideojuegos.get(seleccionar);
            this.listaVideojuegos.remove(seleccionar);
            MostrarTablaVideojuego();
            MeterVideojuegoComboBox();
            CargarDatosPersonajeEnVista();
            crearFichero.EliminarVideojuego(videojuegoEliminado);
        }
    }
    private void jTable_videojuegoMouseClicked(java.awt.event.MouseEvent evt){
        ...
        for(i = 0; i < this.listaSagas.size(); i++){
            if(this.listaSagas.get(i).getIdSaga() == this.listaVideojuegos.get(seleccionar).getIdSagaVideojuego().getIdSaga()){
    ...
    private void ExtraerDatosVideojuego(){
        ...
        for(int i = 0; i < listaSagas.size(); i++){
            ...
            if(listaSagas.get(i).getNombreSaga().equals(jComboBox_saga.getSelectedItem())){
                ...
                idSaga = listaSagas.get(i);
            }    
        }
        
        Videojuego videojuegoAñadido = new Videojuego(nombreVideojuego, motorVideojuego, fechaLanzamiento, pegi, idSaga);
    ...
    private void ModificarDelArrayVideojuegos(){
        ...
        for(i = 0; i < listaVideojuegos.size(); i++){
            ...
            if(listaVideojuegos.get(i).getNombreVideojuego().equals(this.listaVideojuegos.get(seleccionar).getNombreVideojuego())){
                ...
                listaVideojuegos.get(i).setIdSagaVideojuego(saga);
            }
        }
    }
    ...
    private void jButton_guardar_videojuegoActionPerformed(java.awt.event.ActionEvent evt){
        ...
        CambiarEdiccionVideojuego(false);
        MostrarTablaVideojuego();
        this.crearFichero.AniadirOModificarVideojuego(listaVideojuegos);
        seleccionar = -1;
        this.CargarDatosVideojuegoEnVista();
    }
    ...
    private void BorrarSagaDelArray(){
        ...
        boolean posibilidadBorrado = true;
        Saga sagaEliminar = this.listaSagas.get(seleccionar);
        for(int i = 0; i < listaVideojuegos.size(); i++){
            if(listaVideojuegos.get(i).getIdSagaVideojuego().getIdSaga() == listaSagas.get(seleccionar).getIdSaga()){
        ...
        if(posibilidadBorrado){
            ...
            MeterSagaComboBox();
            CargarDatosVideojuegoEnVista();
            crearFichero.EliminarSaga(sagaEliminar);
        }
    }
    private void jTable_sagaMouseClicked(java.awt.event.MouseEvent evt){
        ...
        for(i = 0; i < listaEmpresas.size(); i++){
            ...
            if(listaEmpresas.get(i).getIdEmpresa() == listaSagas.get(seleccionar).getIdEmpresaAsignada().getIdEmpresa()){
    ...
    private void ExtraerDatosSaga(){
        ...
        Saga sagaAñadida = new Saga(nombreSaga, desarrolladora, genero, anio_creacion, empresa);
    ...
    private void ModificarDelArraySagas(){
        ...
        Empresa empresaenSaga = null;
        int i = 0;
        for(i = 0; i < listaEmpresas.size(); i++){
            
            if(listaEmpresas.get(i).getNombre().equals(empresaPertenece))
                empresaenSaga = listaEmpresas.get(i);
        }
        ...
        for(i = 0; i < listaSagas.size(); i++){
            ...
            if(listaSagas.get(i).getNombreSaga().equals(listaSagas.get(seleccionar).getNombreSaga())){
                ...
                listaSagas.get(i).setIdEmpresaAsignada(empresaenSaga);
            }
        }
    }
    ...
    private void jButton_guardar_sagaActionPerformed(java.awt.event.ActionEvent evt){
        ...
        CambiarEdiccionSaga(false);
        MostrarTablaSaga();
        crearFichero.AniadirOModificarSaga(listaSagas);
        seleccionar = -1;
        CargarDatosSagaEnVista();
    }
    ...
    private void BorrarEmpresaDelArray(){
        ...
        for(int i = 0; i < listaSagas.size(); i++){
            if(listaSagas.get(i).getIdEmpresaAsignada().getIdEmpresa() == listaEmpresas.get(seleccionar).getIdEmpresa()){
        ...
        if(posibilidadBorrado){
            empresaBorrada = this.listaEmpresas.get(seleccionar);
            this.listaEmpresas.remove(seleccionar);
            MostrarTablaEmpresa();
            MeterEmpresaComboBox();
            CargarDatosEmpresaEnVista();
            seleccionar = -1;
            crearFichero.EliminarEmpresa(empresaBorrada);
        }
    }
    ...
    private void ExtraerDatosEmpresa(){
        ...
        Empresa empresaAñadida = new Empresa(nombreEmpresa, paisSede, ceo, fechaFundacion);
    ...
    private void jButton_guardar_empresaActionPerformed(java.awt.event.ActionEvent evt){
        ...
        CambiarEdiccionEmpresa(false);
        MostrarTablaEmpresa();
        crearFichero.AniadirOModificarEmpresa(listaEmpresas);
        CargarDatosEmpresaEnVista();
    }
    ...
    public EscribirBDHibernate getCrearFichero() {
        return crearFichero;
    }
